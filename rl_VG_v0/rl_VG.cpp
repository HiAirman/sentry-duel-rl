// rl_VG.cpp - RL 策略部署 AI(哨兵大战参赛 .so)
//
// 权重来源:rl_monet_v1 自对弈训练 → runs/m2/best.npz
...
//          权重在同目录 rl_weights.h
//   - 决策:masked softmax 温度采样(T=1.0,与训练时采样温度一致),行动被引擎拒绝时屏蔽该动作重试
...
#include "rl_weights.h"  // 生成文件:kW0..kB4 + kLN1Gamma..kLN3Beta
...
void rl_act(const Board& board, char my_color) {
    ...
    while (used < 3) {
        ...
        // masked 温度采样
        int best = sample_with_temperature(logits, mask, kDeployTemperature);
        // 二次过滤已 banned 的(采样可能采到 banned)
        if (banned[best] > 0.0f) {
            float lg[kA];
            for (int i = 0; i < kA; ++i) lg[i] = (banned[i] > 0.0f) ? -1e30f : logits[i];
            int tries = 0;
            do {
                best = sample_with_temperature(lg, mask, kDeployTemperature);
            } while (banned[best] > 0.0f && ++tries < 8);
        }
        if (mask[best] <= 0.0f || best == 7) break;  // 不可执行或 end
