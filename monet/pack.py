...
#
# **例外:`m1_v1` 只在这里注册。** 它是"起训权重"而不是对手,所以**不进**
# selfplay 的对手表,更不进联盟/评测名单 —— 一旦进了,联盟配重与评测口径就
# 跟着变了(而且它和前几代同源,当陪练的梯度价值也低)。
KNOWN_PACKS = {
    "rl_v5": "rl_v5-source",  # 外部强 AI,与本仓库并列
    "m3_v1": "m3_v1",         # m3 的冻结导出,基线锚点
    "m3_v2": "m3_v2",         # m3 的另一次冻结导出,比 m3_v1 晚
    "m1_v1": "m1_v1",         # 仓库内;runs/m1/best.npz 的冻结导出(6.1 万步,
                              # 全仓库训练度最低),只当起训权重 —— 见上面的例外
