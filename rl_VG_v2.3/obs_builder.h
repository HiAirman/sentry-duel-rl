...
public:
    ...
    // ---- rl_VG_v1.0 新增:供同目录 vg_rules.h 的规则层读取 ----
    // 规则 1(击杀搜索)要判 `opp_visible` 才知道 intel_pos 是不是敌人**实时**位置,
    // 规则 2(失明计数)要按 `turn` 划阶段边界,两者都要障碍集来算弹道遮挡。
    //
    // 这四个都是**只读**访问器,`encode()` 的 428 维输出一个字节都没动 —— 观测规格
    // 与 rl_VG_v0.2 逐位相同,所以 v0.2 的权重依然能在这个包里跑。
    bool opp_visible() const { return opp_visible_; }
    Pos intel_pos() const { return intel_pos_; }
    int turn() const { return turn_; }
    const std::vector<Pos>& obstacles() const { return obstacles_; }
